          ...
          <thead className="bg-muted/30">
            <tr className="border-b text-xs text-muted-foreground">
              {columns.map((col) => (
                <th
                  key={col.key}
                  className={`text-left py-3 px-4 font-medium ${col.className || ''} ${col.hideOnMobile ? 'hidden md:table-cell' : ''}`}
                >
                  {col.sortable ? (
                    <button
          ...
          <tbody>
            {loading ? (
              ...
              paged.map((row, i) => (
                ...
                >
                  {columns.map((col) => (
                    <td key={col.key} className={`py-3 px-4 ${col.className || ''} ${col.hideOnMobile ? 'hidden md:table-cell' : ''}`}>
                      {col.cell(row)}
                    </td>
                  ))}
                </motion.tr>
