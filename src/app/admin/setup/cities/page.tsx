...
import { MapPin, Plus, Edit, Trash2, MoreVertical, X } from 'lucide-react'
...
export default function Page() {
  ...
  function openEdit(item: Entity) {
    ...
    initial['governorate'] = item['governorate'] || ''
  ...
  async function handleSave() {
    ...
    try {
      ...
      const url = isEdit ? '/api/admin/cities' : '/api/admin/cities'
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/30">
                <tr className="border-b text-xs text-muted-foreground">
                  ...
                  <th className="text-right py-3 px-4 font-medium">المناطق</th>
              ...
              <tbody>
                {data.map((item, i) => (
                  <tr key={item.id || i} className="border-b last:border-0 hover:bg-accent/30 transition-colors">
                    ...
                    <td className="py-3 px-4">{item.governorate || '-'}</td>
                    <td className="py-3 px-4">{item.zonesCount || '-'}</td>
            ...
            >
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input value={form['name'] || ''} onChange={(e) => setForm({ ...form, 'name': e.target.value })} placeholder="القاهرة" />
                </div>
                <div className="space-y-2">
                  <Label>الكود</Label>
                  <Input value={form['code'] || ''} onChange={(e) => setForm({ ...form, 'code': e.target.value })} placeholder="CAI" />
                </div>
                <div className="space-y-2">
                  <Label>المحافظة</Label>
                  <Input value={form['governorate'] || ''} onChange={(e) => setForm({ ...form, 'governorate': e.target.value })} placeholder="القاهرة" />
                </div>
                <div className="space-y-2">
                  <Label>الحالة</Label>
