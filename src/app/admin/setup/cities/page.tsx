...
export default function Page() {
  ...
  useEffect(() => {
    fetch('/api/admin/setup?resource=cities')
      .then(r => r.json())
      .then(d => {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="المدن" subtitle="إدارة المدن المتاحة للشحن" icon={MapPin} />
              ...
              <tbody>
                {data.map((row, i) => (
                  <tr key={i} className="border-b last:border-0 hover:bg-accent/30 transition-colors">
                    ...
                    <td className="py-3 px-4">{getValue(row, 'governorate') || '-'}</td>
                    <td className="py-3 px-4">{getValue(row, 'zonesCount') || '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
