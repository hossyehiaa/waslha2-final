...
import { Shield, Plus, Edit, Trash2, MoreVertical, X } from 'lucide-react'
...
export default function Page() {
  ...
  async function load() {
    ...
    try {
      ...
      setData(d['templates'] || d['templates'] || [])
  ...
  async function handleSave() {
    ...
    try {
      ...
      const url = isEdit ? '/api/admin/permission-templates' : '/api/admin/permission-templates'
  ...
  async function handleDelete() {
    ...
    try {
      const res = await fetch('/api/admin/permission-templates?id=' + deleteTarget.id, { method: 'DELETE' })
              ...
              <tbody>
                {data.map((item, i) => (
                  <tr key={item.id || i} className="border-b last:border-0 hover:bg-accent/30 transition-colors">
                    ...
                    <td className="py-3 px-4">{item.isDefault || '-'}</td>
            ...
            >
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input value={form['name'] || ''} onChange={(e) => setForm({ ...form, 'name': e.target.value })} placeholder="مدير عام" />
                </div>
                <div className="space-y-2">
                  <Label>الوصف</Label>
                  <Textarea value={form['description'] || ''} onChange={(e) => setForm({ ...form, 'description': e.target.value })} placeholder="وصف الصلاحيات" rows={2} />
                </div>
              </div>
