...
export default function AdminApiKeysPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {createdKey && <Card className="p-6 border-green-500 bg-green-50 dark:bg-green-950/30"><h3 className="font-semibold text-green-700 dark:text-green-400 mb-2">Save your API key now</h3><p className="text-sm text-muted-foreground mb-3">This is the only time you will see the full key. Copy it now.</p><div className="flex items-center gap-2"><code className="flex-1 p-3 rounded-lg bg-white dark:bg-black/30 font-mono text-sm break-all">{createdKey}</code><Button size="icon" onClick={() => copyKey(createdKey)}>{copied ? <Check className="w-4 h-4 text-green-500" /> : <Copy className="w-4 h-4" />}</Button></div><Button variant="outline" className="mt-3" onClick={() => setCreatedKey(null)}>I have saved it</Button></Card>}
