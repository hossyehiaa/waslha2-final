...
import { Wallet, Plus, Edit, Trash2, MoreVertical, X } from 'lucide-react'
...
export default function Page() {
  ...
  async function load() {
    ...
    try {
      ...
      setData(d['accounts'] || d['accounts'] || [])
      const brRes = await fetch('/api/admin/branches'); const brD = await brRes.json(); setExtraData({ branches: brD.branches || [] });
  ...
  function openEdit(item: Entity) {
    ...
    initial['type'] = item['type'] || ''
    initial['balance'] = item['balance'] || ''
    initial['branchId'] = item['branchId'] || ''
    initial['isActive'] = item['isActive'] || ''
    setForm(initial)
    setModalOpen(true)
  }
  ...
  async function handleDelete() {
    ...
    try {
      const res = await fetch('/api/admin/accounts?id=' + deleteTarget.id, { method: 'DELETE' })
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="بيانات الحسابات"
        subtitle="إدارة الحسابات المالية"
        icon={Wallet}
        actions={
              ...
              <tbody>
                {data.map((item, i) => (
                  <tr key={item.id || i} className="border-b last:border-0 hover:bg-accent/30 transition-colors">
                    ...
                    <td className="py-3 px-4"><span className="text-xs px-2 py-1 rounded bg-muted">{item.type || '-'}</span></td>
            ...
            >
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input value={form['name'] || ''} onChange={(e) => setForm({ ...form, 'name': e.target.value })} placeholder="الحساب الرئيسي" />
                </div>
                <div className="space-y-2">
                  <Label>النوع</Label>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={form['branchId'] || ''} onValueChange={(v) => setForm({ ...form, 'branchId': v })}>
                    ...
                    <SelectContent>
                      ...
                      {extraData.branches?.map((b: any) => <SelectItem key={b.id} value={b.id}>{b.name}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
